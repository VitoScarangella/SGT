...
class AdminDefaultController extends BaseController
{
	...
	/**
	 * EFX
	 * Lists all models frontend.
	 * @return mixed
	 */
	public function actionIndexfe()
	{
		$this->modelSearchClass = 'webvimark\modules\UserManagement\models\search\UserSearchFe';
		$session = Yii::$app->session;
		$session["userModelSearchClass"] = $this->modelSearchClass;
		return $this->actionIndex();
	}

	/**
	 * EFX
	 * Lists all models backend.
	 * @return mixed
	 */
	public function actionIndexbe()
	{
		$this->modelSearchClass = 'webvimark\modules\UserManagement\models\search\UserSearchBe';
		$session = Yii::$app->session;
		$session["userModelSearchClass"] = $this->modelSearchClass;
		return $this->actionIndex();
	}
	...
	{
		$this->modelSearchClass = 'webvimark\modules\UserManagement\models\search\UserSearch';
		$session = Yii::$app->session;
		$session["userModelSearchClass"] = $this->modelSearchClass;
		return $this->actionIndex();
	}
	...
	/**
	EFX divisione utenti backend da frontend
	 * Lists all models.
	 * @return mixed
	 */
	public function actionIndex()
	{
		...
		{
			$session = Yii::$app->session;
			if (isset($session["userModelSearchClass"])
			&&
			!empty($session["userModelSearchClass"])
			)
			$this->modelSearchClass = $session["userModelSearchClass"];
			else {
				$this->modelSearchClass = 'webvimark\modules\UserManagement\models\search\UserSearch';
			}
		}
	...
	public function actionDelete($id)
	{
		...
		//EFX
		$redirect = $this->getRedirectPage('index', $model); //EFX fix
	...
	protected function getRedirectPage($action, $model = null)
	{
		...
		switch ($action)
		{
			case 'delete':
				...
				break;
			case 'update':
				return Yii::$app->request->isAjax ? false : ['update', 'id'=>$model->id];
				break;
			case 'create':
				return Yii::$app->request->isAjax ? false : ['update', 'id'=>$model->id];
				break;
