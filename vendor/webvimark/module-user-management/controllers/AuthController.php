...
class AuthController extends BaseController
{
	 ...
	 * @var array
	 */
	public $freeAccessActions = ['login', 'logout', 'password-recovery', 'password-recovery-receive', 'confirm-registration-email', 'access-by-token'];
	...
	{
		//EFX beforeLogin
		\backend\controllers\ThisSiteController::beforeLogin();
		...
		{
			//EFX afterLogin
			\backend\controllers\ThisSiteController::afterLogin();
	...
	{
		//EFX beforeLogout
		\backend\controllers\ThisSiteController::beforeLogout();

		Yii::$app->user->logout(true);

		//EFX afterLogout
		\backend\controllers\ThisSiteController::afterLogout();
	...
	public function actionRegistration()
	{
		...
		//EFX Utilizzo il modelle specifico del progetto
		$this->module->registrationFormClass = \backend\controllers\ThisSiteController::$registrationFormClass;
				...
				if ( $this->triggerModuleEvent(UserAuthEvent::AFTER_REGISTRATION, ['model'=>$model, 'user'=>$user]) )
				{
					if ( $user )
					{
						...
						else
						{
							...
							//\backend\controllers\ThisSiteController::beforeLogin(); //EFX
							Yii::$app->user->login($user);
							//\backend\controllers\ThisSiteController::afterLogin(); //EFX
		...
		$registrationForm = \backend\controllers\ThisSiteController::$registrationFormView;


		return $this->renderIsAjax($registrationForm, compact('model'));
	}
	 ...
	 * @return string|\yii\web\Response
	 */
	public function actionAccessByToken($token)
	{
		$user = User::findIdentityByAccessToken($token);

		if ( !$user )
		{
			return $this->renderIsAjax('confirmEmailFailure', compact('user'));
		}
