...
class Role extends AbstractItem
{
	...
	public static function getAvailableRoles($showAll = false, $asArray = false)
	{
		...
		$result = static::find()->andWhere($condition)->andWhere(["=","enabled",1])->all(); //EFX andWhere(["=","enabled",1])  visualizzo solo i ruoli abilitati

		$connection = \Yii::$app->db;

		//EFX visualizzo dettaglio del ruolo
    foreach ($result as $key => $value) {
			$sql = "select description from auth_item_ext where name='".$result[$key]["name"]."'";
			$m = $connection->createCommand($sql);
			$model = $m->queryOne();
			//$result[$key]["description"] = "<b>".$result[$key]["description"]."</b>".$model["description"];
    }
