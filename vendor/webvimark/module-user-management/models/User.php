...
class User extends UserIdentity
{
  ...
  //EFX Dati di dettaglio sui ruoli dell'utente
	public function getRolesdetails()
	{
		$connection = \Yii::$app->db;
		$sql = "
			select auth_item_ext.name, auth_item_ext.description, auth_item.description des
			from auth_item
			join auth_assignment on auth_assignment.user_id=".$this->id ." and  auth_assignment.item_name = auth_item.name
			left join auth_item_ext on auth_item_ext.name = auth_assignment.item_name
		";
    ...
    $s="";
		foreach ($models as $model) {
			if ($model["description"]=="")
				...
				$s .=  "<b>".$model["des"]."</b>:".$model["description"]."<br>";
		}

    $s .= \common\models\Tool::getUserInfo($this->id);
		/*
		$sql = "select EV_TITOLO description from gs_evento, gs_evento_user where idUSer=".$this->id .
		" and gs_evento_user.idEvento = gs_evento.EV_ID ";
		$m = $connection->createCommand($sql);
		$models = $m->query();
		$s.=  "<p style='color:blue' >";
		foreach ($models as $model) {
			$s.=  "<br>" . $model["description"];
		}
		$s.=  "</p>";
		*/
	...
	/**
     * EFX verifica se posso cancellare user
	 * @return \yii\db\ActiveQuery
	 */
	public function getCandelete()
	{
        return \backend\controllers\ThisSiteController::canDeleteUser($this);
	}


	//EFX ALlinea Userdetails
	public function afterSave($insert, $changedAttributes)
  {
		 \backend\controllers\ThisSiteController::afterUserSave($this);
  }
	...
	public function beforeDelete()
	{
		...
		if ( php_sapi_name() != 'cli' )
		{
			...
			//EFX beforeUSerDelete prima della cancellazione di un utente
			\backend\controllers\ThisSiteController::beforeUserDelete($this, $this);
			}
