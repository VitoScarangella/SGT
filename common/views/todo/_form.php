...
<div class="todo-form">
    <?php $form = ActiveForm::begin(['type'=>ActiveForm::TYPE_VERTICAL]); echo FormGrid::widget([
		...
		'attributes' => [
            'stato'=>['type'=> Form::INPUT_WIDGET, 'widgetClass'=>'\kartik\widgets\Select2','options'=>[
                 'data'=>Tool::dropdown('(select "0" id, "APERTO" codice union select "1" id, "CHIUSO" codice  union select "2" id, "SOSPESO" codice) combo_yn','id','codice'), 'size' => Select2::LARGE,
              ]],
		...
		'attributes' => [
            'priorita'=>['type'=> Form::INPUT_WIDGET, 'widgetClass'=>'\kartik\widgets\Select2','options'=>[
                 'data'=>Tool::dropdown('(select "0" id, "BASSA" codice union select "1" id, "MEDIA" codice  union select "2" id, "ALTA" codice union select "3" id, "BLOCCANTE" codice) combo_yn','id','codice'), 'size' => Select2::LARGE,
              ]],
    ...
    echo $model->isNewRecord ? "" : "&nbsp;&nbsp;" .  Html::button('Delete',
        [ 'class' => 'btn btn-primary',
          ...
          location.href="'
            . Yii::$app->urlManager->createUrl('todo/delete')
            .'&id='.$model->id.'";',
        ]);
